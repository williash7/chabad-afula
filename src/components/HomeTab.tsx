...
export function HomeTab({ setTab, onDonationClick }: { setTab: (t: string) => void, onDonationClick: () => void }) {
  ...
  const { personalAlerts, allPersonalEvents, crmAlerts } = React.useMemo(() => {
    ...
    const isEndOfWeek = today.getDay() === 4 || today.getDay() === 5; // Thu/Fri

    // Check next holiday for target invites
    let nextHolDays = 999;
    ...
    const cleanHeStr = (str: string) => {
       // Keeps digits and hebrew letters and spaces, removes all Niqqud, punctuation, tags, etc.
       return String(str).replace(/[\u0591-\u05C7\u200E\u200F]/g, '').replace(/[^\u05D0-\u05EA0-9\s]/g, ' ').replace(/\s+/g, ' ').trim();
    };

    const nextYearHebrew = new Map<string, number>();
    for(let i=0; i<400; i++) { // cover a full leap year + some
        const d = new HDate(new Date(today.getTime() + i*86400000));
        
        // Format 1: Gematria
        const gem = d.renderGematriya ? d.renderGematriya() : d.render('he');
        let partsGem = gem.split(' ');
        partsGem.pop(); // remove year
        let keyGem = cleanHeStr(partsGem.join(' ')); 
        const pGem = keyGem.split(' ');
        if (pGem.length > 1 && pGem[1].startsWith('ב') && pGem[1].length > 1) {
            keyGem = pGem[0] + ' ' + pGem[1].substring(1);
        }
        if (!nextYearHebrew.has(keyGem)) nextYearHebrew.set(keyGem, i);

        // Format 2: Numbers ("16 אייר")
        const numHe = d.render('he'); 
        let partsNum = numHe.split(','); 
        let keyNum = cleanHeStr(partsNum[0]); 
        const pNum = keyNum.split(' ');
        if (pNum.length > 1 && pNum[1].startsWith('ב') && pNum[1].length > 1) {
            keyNum = pNum[0] + ' ' + pNum[1].substring(1);
        }
    ...
    Object.keys(donors).forEach(name => {
      ...
      if (!hasPersonalAlert && gBday) {
        ...
        if (d > 0 && m > 0 && d <= 31 && m <= 12) {
           ...
           if (realDays >= 0) {
              allPersonalEvents.push({ type: 'info', name, msg: 'יום הולדת ' + (realDays===0?'היום':`בעוד ${realDays} ימים`), action: '🎁 ברך', icon: '🎈', priority: 1, dist: realDays });
           }
        }
      }
      ...
      // CRM Alerts
      const data = crm[name];
      if (data?.target && !hasPersonalAlert) { // if no personal alert, maybe add a CRM alert
         if (nextHolDays <= 14) {
             cAlerts.push({ type: 'info', name, msg: 'חג מתקרב — הזמן אישית!', action: '💬 שלח', icon: '🍷', priority: 2 });
         } else if (isEndOfWeek && Math.random() > 0.5) {
             cAlerts.push({ type: 'target', name, msg: 'התקשר לאחל שבת שלום', action: '📞 התקשר', icon: '🕯️', priority: 3 });
         } else {
             cAlerts.push({ type: 'target', name, msg: 'מסומן להקרב — הצע חברותא או פגישה', action: '📝 תכנן', icon: '🎯', priority: 4 });
         }
      }
    });
    ...
    return { 
       personalAlerts: allPersonalEvents.filter(a => a.dist <= 30).slice(0, 10), 
       allPersonalEvents,
       crmAlerts: cAlerts 
    };
  }, [donors, crm, holidays]);
            ...
            <table className="w-full text-right text-sm">
              <thead className="bg-gray-50 border-b border-[#EDE6D6] text-gray-500 text-[11px] uppercase tracking-wide">
                <tr>
                  ...
                  <th className="py-2 px-3 font-semibold">שם תורם/מתפלל</th>
                  ...
                  <tr key={i} className="hover:bg-gray-50/50 transition-colors">
                    <td className="py-2 px-3">
                      <span className="flex items-center gap-1.5 text-[#0D1B2A]">
                        ...
                        <span className="max-w-[70px] truncate" title={a.msg.split(' ')[0]}>{a.msg.split(' ')[0]} {a.msg.split(' ')[1]}</span>
                      </span>
                    </td>
                    ...
                    <td className="py-2 px-3 text-center">
                      <button 
                        onClick={() => setSelectedDonor(a.name)}
                        className="bg-[#C9A84C]/10 text-[#9B7A2F] text-[10px] font-bold px-2.5 py-1 rounded-md active:scale-95 transition-transform whitespace-nowrap"
                      >
                        {a.action}
                      </button>
                    </td>
      ...
      <div className="space-y-2">
        {crmAlerts.slice(0, 5).map((a, i) => (
          <div key={i} className="bg-white border border-[#EDE6D6] rounded-xl p-3 flex items-center gap-3 shadow-sm">
            ...
            <button 
              onClick={() => setSelectedDonor(a.name)}
              className="bg-[#0D1B2A] text-[#E8C97A] text-[11px] font-bold px-3 py-1.5 rounded-lg shrink-0"
            >
              {a.action}
            </button>
          </div>
  ...
  const renderShabbatCard = () => {
    ...
    return (
      <div className="bg-gradient-to-br from-[#1B0042] to-[#3B0082] rounded-2xl p-4 mb-4 text-white flex items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <span className="text-3xl">🕯️</span>
          <div className="flex-1">
            <div className="font-['Frank_Ruhl_Libre'] text-base font-bold">
              {parasha?.hebrew || parasha?.title || 'פרשת השבוע'}
            </div>
            <div className="flex gap-4 mt-1">
              <div className="text-center">
                <div className="font-['Frank_Ruhl_Libre'] text-lg font-bold text-[#E8C97A]">
                  {formatTime(candles?.date)}
                </div>
                <div className="text-[10px] text-white/50 mt-[1px]">הדלקת נרות</div>
              </div>
              <div className="text-center">
                <div className="font-['Frank_Ruhl_Libre'] text-lg font-bold text-[#E8C97A]">
                  {formatTime(havdala?.date)}
                </div>
  ...
  const renderHolidaysAndTasks = () => {
    ...
    holidays.forEach(h => {
      ...
      if (dateStr) allHols.push({ name: h.hebrew || h.title, dateStr, emoji: '✡️' });
    });
    getCustomHols().forEach((c: any) => {
      allHols.push({ name: c.name, dateStr: c.date, emoji: '📅' });
    });
              ...
              <div key={i} className="bg-white rounded-xl p-3 border border-[#EDE6D6] shadow-sm">
                ...
                {openTasks.length > 0 ? (
                  ...
                  <div className="text-xs text-green-600 flex items-center gap-1 mt-1">
                    <CheckCircle size={12} /> כל המשימות הושלמו!
                  </div>
                ) : (
                  <div className="text-xs text-gray-400 mt-1 italic">
                    אין משימות מתוכננות לחג זה
                  </div>
                )}
              </div>
            );
  ...
  const renderRebbeCard = () => {
    ...
    return (
      <div className="bg-gradient-to-br from-[#1a0a00] to-[#3d1f00] rounded-2xl p-4 mb-4 text-white relative overflow-hidden group">
        ...
        <div className="flex justify-between items-start relative z-10">
          <div>
            ...
            {diff !== null && (
              <div className={`inline-block rounded-lg px-2.5 py-1 text-xs mt-2 border ${bg}`}>
                {urgency}לפני {diff} ימים
              </div>
            )}
            <div className="mt-4">
              <a href="mailto:ohel@ohelchabad.org?subject=כתיבה לרבי" className="inline-block bg-[#E8C97A] text-[#1a0a00] font-bold text-xs px-4 py-2 rounded-lg transition-transform hover:scale-105 active:scale-95 shadow-sm">
                שלח מייל עכשיו ✉️
              </a>
            </div>
          </div>
          <button 
            onClick={() => setIsRebbeEditOpen(true)}
    ...
    <div className="animate-in fade-in pb-24">
      ...
      <div className="bg-[#0D1B2A] px-4 py-3 pb-3 flex items-center justify-between sticky top-0 z-50 shadow-md">
        ...
        <div className="flex-1 px-3">
          <div className="font-['Frank_Ruhl_Libre'] text-lg font-bold text-[#C9A84C]">בית חבד עפולה</div>
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="bg-gradient-to-br from-[#0D1B2A] to-[#1A2E45] rounded-2xl p-5 relative overflow-hidden text-white shadow-lg">
          ...
          <div className="flex items-center justify-between mb-1">
            <div 
              className="text-[11px] text-white/60 uppercase tracking-widest flex items-center gap-1.5 cursor-pointer"
          ...
          <div className="flex flex-wrap gap-2 mt-3">
            {summary?.byMethod && Object.entries(summary.byMethod).map(([method, amount], i) => (
              <div key={method} onClick={() => setSelectedMethodForDetails(method)} className="bg-white/5 border border-white/10 rounded-full px-3 py-1 text-[11px] text-white/65 flex items-center gap-1.5 cursor-pointer hover:bg-white/10 transition-colors">
                ...
                <span className="font-['Frank_Ruhl_Libre'] font-bold text-white tracking-wide">₪{amount.toLocaleString()}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          <h2 className="font-['Frank_Ruhl_Libre'] text-lg font-bold text-[#0D1B2A] mb-3 flex items-center gap-2">
            <span className="text-xl">📋</span> לטיפול עכשיו
          </h2>
          {renderCRMAlerts()}
        </div>
        ...
        {failures.length > 0 && (
          <div>
            <div className="flex justify-between items-center mb-3">
              <h2 className="font-['Frank_Ruhl_Libre'] text-lg font-bold text-[#0D1B2A] text-red-600 flex items-center gap-2">
                ...
                <div key={i} className="bg-white rounded-xl p-3 flex items-center gap-3 shadow-sm border border-[#EDE6D6]">
                  ...
                  <div className="text-left shrink-0 flex flex-col items-end">
                    ...
                    <button 
                      onClick={(e) => {
                        e.stopPropagation();
                        let p = (crm[d.name]?.phone || '');
                        setThankYouInfo({ name: d.name, amount: d.amount || 0, phone: p });
                      }}
                      className="bg-green-50 text-green-600 p-1 rounded hover:bg-green-100 transition-colors"
      ...
      {/* Editing Modal for Rebbe */}
      {isRebbeEditOpen && (
        <div className="fixed inset-0 bg-black/50 z-[200] flex items-end justify-center" onClick={(e) => e.target === e.currentTarget && setIsRebbeEditOpen(false)}>
            ...
            <form onSubmit={handleRebbeSave}>
              <div className="mb-4">
                ...
                <input 
                  ...
                  name="date"
                  required
                  defaultValue={rebbeDate?.toISOString().split('T')[0]}
              ...
              <button 
                type="submit"
                className="w-full bg-gradient-to-br from-[#C9A84C] to-[#9B7A2F] text-white rounded-xl p-3.5 font-['Frank_Ruhl_Libre'] text-lg font-bold shadow-md active:scale-95 transition-transform"
              >
                עדכן תאריך
              </button>
            </form>
      ...
      {selectedMethodForDetails && (
        <div className="fixed inset-0 bg-black/50 z-[200] flex items-end justify-center" onClick={(e) => e.target === e.currentTarget && setSelectedMethodForDetails(null)}>
          <div className="bg-[#FAF6EE] rounded-t-3xl p-5 pb-10 w-full max-w-[430px] max-h-[85vh] overflow-y-auto animate-in slide-in-from-bottom duration-300">
            <div className="flex justify-between items-center mb-5">
              <h2 className="font-['Frank_Ruhl_Libre'] text-xl font-bold text-[#0D1B2A] flex items-center gap-2">
                {selectedMethodForDetails === 'failures_modal' ? <span className="text-red-600 flex items-center gap-2"><AlertTriangle size={20} /> שגיאות חיוב אחרונות</span> : `תרומות מ: ${selectedMethodForDetails}`}
              </h2>
              <button onClick={() => setSelectedMethodForDetails(null)} className="p-2 bg-white rounded-full shadow-sm">
                <X size={16} />
              </button>
            </div>
            <div className="space-y-3">
              {selectedMethodForDetails === 'failures_modal' ? (
                 // Render failures
                 failures.map((f, i) => (
                   <div key={i} className="bg-red-50 rounded-xl p-3 shadow-sm border border-red-100 flex items-center justify-between cursor-pointer active:scale-95 transition-transform" onClick={() => setSelectedDonor(f.name)}>
                     <div>
                       <div className="font-bold text-red-900 text-sm">{f.name}</div>
                       <div className="text-xs text-red-600 mt-0.5">{f.date} · {f.reason}</div>
                     </div>
                     <div className="font-['Frank_Ruhl_Libre'] font-bold text-lg text-red-600 shrink-0 mr-3">
                       ₪{(f.amount)}
                     </div>
                   </div>
                 ))
              ) : (
                // Render donations by method
                donations.filter(d => d.method === selectedMethodForDetails).slice().reverse().map((d, i) => (
                  <div key={i} className="bg-white rounded-xl p-3 shadow-sm border border-gray-100 flex items-center justify-between cursor-pointer active:scale-95 transition-transform" onClick={() => setSelectedDonor(d.name)}>
                    <div>
                    ...
                    <div className="flex flex-col items-end shrink-0 mr-3">
                      <div className="font-['Frank_Ruhl_Libre'] font-bold text-lg text-[#C9A84C]">
                      ...
                      <button 
                        ...
                        className="bg-green-50 text-green-600 p-1.5 rounded-lg mt-1 hover:bg-green-100 transition-colors"
                        title="שלח הודעת תודה"
                      >
                        <MessageSquare size={14} />
                      </button>
      ...
      {/* All Events Modal */}
      {isAllEventsOpen && (
        <div className="fixed inset-0 bg-black/50 z-[200] flex items-end justify-center" onClick={(e) => e.target === e.currentTarget && setIsAllEventsOpen(false)}>
          <div className="bg-[#FAF6EE] rounded-t-3xl p-5 pb-10 w-full max-w-[430px] max-h-[85vh] overflow-y-auto animate-in slide-in-from-bottom duration-300">
            <div className="flex justify-between items-center mb-5 sticky top-0 bg-[#FAF6EE] py-2 z-10">
              <h2 className="font-['Frank_Ruhl_Libre'] text-xl font-bold text-[#0D1B2A] flex items-center gap-2">
                <span className="text-2xl">🎂</span> כל אירועי ימי ההולדת והיארצייט
              </h2>
              <button onClick={() => setIsAllEventsOpen(false)} className="p-2 bg-white rounded-full shadow-sm">
                <X size={16} />
              </button>
            </div>
            ...
            <div className="space-y-3">
              {allPersonalEvents.map((a: any, i: number) => (
                <div key={i} className="bg-white rounded-xl p-3 shadow-sm border border-gray-100 flex items-center justify-between cursor-pointer active:scale-95 transition-transform" onClick={() => setSelectedDonor(a.name)}>
                  ...
                  <div className="text-[#C9A84C] text-[10px] font-bold bg-[#C9A84C]/10 px-2.5 py-1 rounded-md shrink-0">
                    {a.action}
                  </div>
                </div>
              ))}
              {allPersonalEvents.length === 0 && (
                <div className="text-center py-6 text-gray-500 text-sm">אין עדיין ימי הולדת או יארצייט במערכת.</div>
              )}
            </div>
          </div>
        </div>

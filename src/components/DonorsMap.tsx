...
const AFULA_CENTER: [number, number] = [32.6017, 35.2872];
...
export function DonorsMap({ donors }: { donors: DonorPin[] }) {
  ...
  useEffect(() => {
    ...
    const map = L.map(containerRef.current).setView(AFULA_CENTER, 13);
        ...
        if (point) {
          const marker = L.marker([point.lat, point.lon]).bindPopup(
            `<div style="text-align:right;font-family:Heebo,sans-serif;min-width:140px">
              ...
              <a href="https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(d.address + ' עפולה')}"

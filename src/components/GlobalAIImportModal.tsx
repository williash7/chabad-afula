...
export function GlobalAIImportModal({ onClose }: { onClose: () => void }) {
  ...
  const buildPrompt = () => {
    const parts: string[] = [];
    parts.push('אני הרב/השליח בבית חב"ד. יש לי רשימת משימות שונות ואני רוצה שתעזור לי לארגן אותה — פשוט תכתוב לי רשימה (חופשי, כל דבר שעולה לך בראש) ותשייך כל משימה למקום הכי הגיוני: חג, אירוע קבוע, מערך ביקורי בית, אדם ספציפי, או משימה כללית.');

  ...
  useEffect(() => {
    if (shabbat?.items) {
      ...
      if (parashaItem) {
        ...
        fetch('https://www.hebcal.com/shabbat?cfg=json&city=Afula&lg=ru&m=30')
          ...
          .then(data => {
             ...
             if (translatedParasha) {
                let name = translatedParasha.title.replace('Глава ', '');
                setParasha(`Недельная глава: ${name}`);
             } else {
                setParasha(`Недельная глава: ${parashaItem.title.replace('Parashat ', '')}`);
             }
          }).catch(() => {
             setParasha(`Недельная глава: ${parashaItem.title.replace('Parashat ', '')}`);
          });
      ...
      if (candleItem) {
        ...
        setDateFriday(`Пятница, ${hdayFr.getDate()} ${translateHebMonth(hdayFr.getMonthName())} / ${d.getDate()} ${monthsRu[d.getMonth()]}`);
      }
      ...
      if (havdalahItem) {
        ...
        setDateShabbat(`Суббота, ${hdShab.getDate()} ${translateHebMonth(hdShab.getMonthName())} / ${d.getDate()} ${monthsRu[d.getMonth()]}`);
      }
    }
  }, [shabbat]);

  const translateHebMonth = (m: string) => {
    const map: any = {
               ...
               <div className="relative z-20 w-full h-full flex flex-col pt-10">
                  ...
                  <div className="absolute top-0 left-0 w-full flex justify-between items-start px-24 z-30">
                     ...
                     <div className="bg-[#ffffff] rounded-b-[2rem] px-8 py-6 pb-8 flex flex-col items-center min-w-[280px]" style={{ boxShadow: '0 12px 30px rgba(0,0,0,0.15)' }}>
                         <div className="w-full h-14 bg-[#6B1A28] mb-3 flex items-center justify-center text-[#ffffff] text-[32px] font-black rounded-tl-2xl rounded-br-2xl border-2 border-[#4A0A16] border-b-[5px] tracking-wider leading-none pb-1 relative overflow-hidden">
                           ...
                           <span className="relative z-10">חב"ד</span>
                         </div>
                         <div className="text-[#1f2937] text-[18px] text-center leading-relaxed font-sans font-medium">
                            Русскоязычная община<br/>
                            в Афуле Хабад бе-Алия<br/>
                            ул. Бренер 2<br/>
                            <span className="font-bold text-[22px] mt-2 block text-[#6B1A28]">052-5964644</span>
                         </div>
                     </div>
                  </div>
                  ...
                  <div className="px-24 mt-8 flex flex-col items-start gap-4">
                     <div className="bg-[#ffffff] px-10 py-3 rounded-lg transform -rotate-2" style={{ boxShadow: '0 8px 25px rgba(0,0,0,0.15)' }}>
                        <h1 className="text-[110px] leading-none font-bold text-[#6B1A28] pb-2" style={{ fontFamily: 'Georgia, serif', fontStyle:'italic' }}>Время</h1>
                     </div>
                     <div className="bg-[#ffffff] px-10 py-3 rounded-lg transform rotate-1 ml-16" style={{ boxShadow: '0 8px 25px rgba(0,0,0,0.15)' }}>
                        <h1 className="text-[110px] leading-none font-bold text-[#6B1A28] pb-2" style={{ fontFamily: 'Georgia, serif', fontStyle:'italic' }}>молитв</h1>
                     </div>
                  </div>
                  ...
                  <div className="mt-8 px-24 flex items-start justify-between pb-24 relative z-20">
                     ...
                     <div className="w-[460px] flex flex-col gap-3">
                        ...
                        <h3 className="text-[#d78f3c] text-[20px] font-bold mb-1 tracking-widest uppercase text-center">{dateFriday || 'Пятница,...'}</h3>
                        <div className="flex flex-col items-center mb-3 pt-0 bg-white rounded-2xl py-2" style={{ boxShadow: '0 5px 20px rgba(0,0,0,0.06)' }}>
                           <div className="text-[#6B1A28] text-[24px] font-bold leading-none mb-1">Зажжение свечей</div>
                        ...
                        <div className="flex justify-between items-end border-b-[2px] border-[#d1d5db] pb-1 w-[460px]">
                           <span className="text-[22px] text-[#551120] font-bold tracking-wide">Минха</span>
                        ...
                        <div className="flex justify-between items-end border-b-[2px] border-[#d1d5db] pb-1 w-[460px]">
                           <span className="text-[22px] text-[#551120] font-bold tracking-wide">Кабалат Шабат</span>
                           <span className="text-[22px] text-[#551120] dotted-leader flex-1 mx-4 border-b-[3px] border-dotted border-[#aaa] opacity-60 mb-2 block"></span>
                           <span className="text-[24px] font-black text-[#1f2937] leading-none mb-1">{kabbalatShabbat}</span>
                        </div>
                        <div className="flex justify-between items-end border-b-[2px] border-[#d1d5db] pb-1 w-[460px]">
                           <span className="text-[22px] text-[#551120] font-bold tracking-wide">Кидуш</span>
                           <span className="text-[22px] text-[#551120] dotted-leader flex-1 mx-4 border-b-[3px] border-dotted border-[#aaa] opacity-60 mb-2 block"></span>
                           <span className="text-[24px] font-black text-[#1f2937] leading-none mb-1">{kiddush}</span>
                        </div>

                        <h3 className="text-[#d78f3c] text-[20px] font-bold mt-4 mb-1 tracking-widest uppercase text-center">{dateShabbat || 'Суббота,...'}</h3>
                        
                        <div className="flex justify-between items-end border-b-[2px] border-[#d1d5db] pb-1 w-[460px]">
                           <span className="text-[22px] text-[#551120] font-bold tracking-wide">Урок Хасидут</span>
                           <span className="text-[22px] text-[#551120] dotted-leader flex-1 mx-4 border-b-[3px] border-dotted border-[#aaa] opacity-60 mb-2 block"></span>
                           <span className="text-[24px] font-black text-[#1f2937] leading-none mb-1">{chassidut}</span>
                        </div>
                        <div className="flex justify-between items-end border-b-[2px] border-[#d1d5db] pb-1 w-[460px]">
                           <span className="text-[22px] text-[#551120] font-bold tracking-wide">Шахарит</span>
                           <span className="text-[22px] text-[#551120] dotted-leader flex-1 mx-4 border-b-[3px] border-dotted border-[#aaa] opacity-60 mb-2 block"></span>
                        ...
                        <div className="flex justify-between items-end w-[460px] mt-1">
                           <span className="text-[22px] text-[#551120] font-bold tracking-wide">Седер Нигуним</span>
                        </div>
                        
                        <div className="flex flex-col items-center mt-3 bg-white rounded-2xl py-2" style={{ boxShadow: '0 5px 20px rgba(0,0,0,0.06)' }}>
                           <div className="text-[#6B1A28] text-[24px] font-bold leading-none mb-1">Исход Шаббата</div>
                     ...
                     <div className="w-[300px] flex flex-col gap-6 -mt-20 z-20">
                        ...
                        {isMevarchim && (
                           <div className="bg-[#DDE4E4] rounded-3xl p-5 flex flex-col items-center justify-center border border-[#e5e7eb] relative overflow-hidden" style={{ boxShadow: '0 10px 40px rgba(0,0,0,0.1)' }}>
                              ...
                              <div className="text-[#6B1A28] text-[26px] font-bold text-center leading-tight mb-2 relative z-10">Теилим<br/>"Шабат Мевархим"</div>
                        ...
                        <div className="bg-[#DDE4E4] rounded-[2rem] px-5 py-6 relative flex flex-col items-center justify-center text-center border-2 border-white overflow-hidden" style={{ boxShadow: '0 10px 40px rgba(0,0,0,0.1)' }}>
                           ...
                           <div className="text-[#6B1A28] text-[32px] font-bold leading-[1.1] mb-2 relative z-10" style={{ textShadow: "0px 1px 1px rgba(255,255,255,0.5)" }}>Кидуш и<br/>фарбренген</div>
                           <div className="text-[#1f2937] text-[14px] font-black uppercase tracking-widest relative z-10">после утренней молитвы</div>
                        </div>
                        ...
                        {includeClass && (
                           <div className="bg-[#F8F5EE] rounded-[2rem] px-5 py-6 relative flex flex-col items-center justify-center text-center border-2 border-white overflow-hidden" style={{ boxShadow: '0 10px 40px rgba(0,0,0,0.1)' }}>
                              ...
                              <div className="text-[#6B1A28] text-[26px] font-bold leading-none mb-2 relative z-10">Урок по Аалахе</div>

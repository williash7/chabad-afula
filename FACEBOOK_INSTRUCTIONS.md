# הוראות: פיצ'ר פרסום פייסבוק

## מה לבנות
קומפוננטה `FacebookPostAssistant` שדומה ל-`AIPlanningAssistant` הקיימת.
זרימה: מלא פרטים → צור פרומפט → העתק ל-AI → קבל פוסט → הדבק בחזרה → לחצן "פרסם לפייסבוק".

**שני סוגי פוסטים:**
- פוסט ארוך ברוסית → עולה לפייסבוק
- סטטוס קצר בעברית → עולה לפייסבוק (פוסט נפרד)
...
## שלב 1: הגדרות (`src/lib/settings.ts`)

הוסף לממשק `AppSettings` שני שדות חדשים:
```ts
fbPageId: string;      // מזהה דף הפייסבוק
fbAccessToken: string; // טוקן גישה לדף
```

בפונקציית `loadSettings` הוסף ערכי ברירת מחדל:
```ts
fbPageId: '',
fbAccessToken: '',
```
...
## שלב 2: צור `src/components/FacebookPostAssistant.tsx`
...
// שדות לאחרי אירוע בלבד:
const [attendeeCount, setAttendeeCount] = useState('');
const [highlight, setHighlight] = useState('');
// תיבות הדבקה — אחת לפייסבוק ואחת לסטטוס:
...
1. **פוסט פייסבוק ברוסית** (שפה: רוסית בלבד):
   - ארוך, חם, מזמין
...
## שלב 3: שלב ב-`EventsTab.tsx`

ב-Tasks Modal, **אחרי** קומפוננטת `AIPlanningAssistant`, הוסף:

```tsx
import { FacebookPostAssistant } from './FacebookPostAssistant';

// בתוך AppContent, קח settings מה-store:
const { ..., settings } = useAppStore();

// בתוך ה-Tasks Modal JSX, אחרי </AIPlanningAssistant>:
...
## שלב 4: הוסף שדות ב-`SettingsTab.tsx`

בסוף דף ההגדרות, הוסף כרטיס חדש "הגדרות פייסבוק":

```tsx
const { settings, updateSettings } = useAppStore();

// JSX:
<div className="bg-white rounded-xl p-4 border border-[#EDE6D6] space-y-3 mt-4">
  <div className="text-sm font-bold text-[#0D1B2A] flex items-center gap-2">
    <svg width="16" height="16" viewBox="0 0 24 24" fill="#1877F2">...</svg>
    הגדרות פייסבוק
  </div>
  <p className="text-[11px] text-gray-500">
    למילוי שדות אלה: כנס ל-developers.facebook.com, צור אפליקציה חינמית, קבל Page Access Token עבור הדף שלך.
    (מדריך מפורט — שאל את Claude)
  </p>
  <div>
...
## שלב 5: בדיקה לאחר הבנייה

1. הרץ `npm run build` — ודא אין שגיאות TypeScript
2. פתח אירוע → לחץ "משימות" → ודא שמופיע כפתור "פרסום לפייסבוק" בתחתית
3. לחץ → בחר "לפני האירוע" → לחץ "צור פרומפט" → ודא שהפרומפט נבנה ומכיל פרטי האירוע
4. ודא שסעיף ההגדרות מציג את שדות פייסבוק
...
| קובץ | פעולה |
|------|--------|
| `src/components/FacebookPostAssistant.tsx` | **צור חדש** |
| `src/components/EventsTab.tsx` | הוסף import + קומפוננטה בתוך Tasks Modal |
| `src/components/SettingsTab.tsx` | הוסף כרטיס הגדרות פייסבוק |
| `src/lib/settings.ts` | הוסף `fbPageId` ו-`fbAccessToken` ל-AppSettings |
